...
public class IdProdutoDAO {

    public int usuarioID() {
        int aux = 0;
        try {
            BufferedReader carregar = new BufferedReader(new FileReader("idProdutos.txt"));
    ...
    public void salvar(String tmp1) {
        try {
            BufferedWriter salvar = new BufferedWriter(new FileWriter("iProdutos.txt"));
            salvar.write(tmp1);
            salvar.newLine();
            salvar.close();
        } catch (IOException e) {
